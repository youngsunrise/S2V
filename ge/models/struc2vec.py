...
class Struc2Vec():
    def __init__(self, graph, walk_length=10, num_walks=100, workers=1, verbose=0, stay_prob=0.3, opt1_reduce_len=True,
        ...
        if not reuse:
            ...
            os.mkdir(self.temp_path)
        # print("num of layers is ===================", self.opt3_num_layers)
        layersumProb = self.create_context_graph(self.opt3_num_layers, workers, verbose)
        ...
        self._embeddings = {}
        self.layersumProb = layersumProb
    ...
    def create_context_graph(self, max_num_layers, workers=1, verbose=0, ):
        ...
        layers_accept, layers_alias, layersunProb = self._get_transition_probs(
    ...
    def _get_order_degreelist_node(self, root, max_num_layers=None):
        ...
        while (len(queue) > 0 and level <= max_num_layers):
            ...
            level += 1
        # print("num of layers is ===================", max_num_layers)
        # print("num of level is ===================", level)
        #############################################################################################
    ...
    def _get_transition_probs(self, layers_adj, layers_distances):
        ...
        #### context graph (2023.7.4 yxs)
        sum_normalWeights = {}
        N = len(layers_adj[0].items())
        layersumProb = np.zeros(N * N).reshape(N, N)
        ####
        ...
        for layer in layers_adj:
            ...
            sumProb = np.zeros(N * N).reshape(N, N)
            ...
            for v, neighbors in neighbors.items():
                ...
                for n in neighbors:
                    ...
                    sumProb[v][n] = w
##############################################################################
                # # 用sumProb存储节点和邻居们正则化的权值
                # for n in neighbors:
                #     sumProb[v][n] = sumProb[v][n] / sum_w
                # #
##############################################################################
                e_list = [x / sum_w for x in e_list]
                norm_weights[v] = e_list               # 正则化的边权值
                accept, alias = create_alias_table(e_list)
                node_alias_dict[v] = alias
                node_accept_dict[v] = accept
            ### 到此步，第k层的所有节点在sumprob内存储的边权值初始化完成
            #### context graph (2023.7.4 yxs)
            layersumProb += sumProb
            # print("layer is ", layer)
            # print("sumProb is ", sumProb)
        ...
        # print("shape of layersumProb is ", np.shape(layersumProb))
        # print("layersumProb is ", layersumProb)

        return layers_accept, layers_alias, layersumProb
